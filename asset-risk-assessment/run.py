                ...
                if len(results) > 0:
                    ...
                    for asset in results:
                        if asset['id'] in asset_report.keys():
                            asset_report[asset['id']]['risks'].append(search_name)
                        else:
...
def main():
    ...
    for o in orgs.json():
        ...
        if not token:
            print(f'skipping {name} - you need to enable the export token in the UI to run the report')
        elif asset_count > max_assets:
            print(f'skipping {name} - you need to increase the max_assets value to be above {max_assets} to run')
