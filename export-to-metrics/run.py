...
SEARCHES = [
    ...
    },
    {
        ...
        "denominator": "alive:t _asset.protocol:tls",
    },
    {
...
def handle_search(search, type):
    url = BASE_URL + "/export/org/assets.json" if type == "assets" else BASE_URL + "/export/org/services.json"
    data = requests.get(url, headers=HEADERS, params={"search": "alive:t " + search, "fields": "id"})
    return data
def main():
    ...
    for search in SEARCHES:
        ...
        denominator_data = handle_search(search=search["denominator"], type=search["type"])
        ...
        search_link = "https://console.runzero.com/inventory?search=" if search["type"] == "assets" else "https://console.runzero.com/inventory/services?search="
        for i, id in enumerate(asset_ids):
            if i == 0:
                search_link += "service_id:" + id if search["type"] == "services" else "id:" + id
            else:
                search_link += (
