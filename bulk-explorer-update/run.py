...
def update_explorer(explorer_id: str):
    ...
    update = requests.patch(url, headers=HEADERS, json={
...
def main():
    ...
    for a in explorers:
        ...
        if site_id == "00000000-0000-0000-0000-000000000000":
            ...
            if update <= 202:
                print(
                    f"Successfully updated explorer {explorer_id} to {DEFAULT_SITE}")
            else:
                print(
                    f"Failed to update explorer {explorer_id} - status code: {update}")
        else:
